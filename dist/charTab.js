...
import { decodeRleToRgba, decodeRawToRgba, rgbaToDataUrl } from './imageDecode.js';
...
export async function loadWithNewProtocol() {
...
function handleDeleteSet(setName) {
    if (!appState.imagesData)
        return;
    if (!confirm(`「${setName}」を削除しますか？（そのキャラの設定がすべて消えます。画像自体は削除されません）`))
        return;
