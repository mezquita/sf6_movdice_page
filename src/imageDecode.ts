...
function rgb565ToRgb888(val: number): [number, number, number] {
...
export function decodeRleToRgba(bytes: Uint8Array): Uint8ClampedArray {
...
export function decodeRawToRgba(bytes: Uint8Array): Uint8ClampedArray {
  const pixelCount = IMG_WIDTH * IMG_HEIGHT;
  const rgba = new Uint8ClampedArray(pixelCount * 4);
...
export function rgbaToDataUrl(rgba: Uint8ClampedArray): string {
  ...
  ctx.putImageData(new ImageData(rgba as Uint8ClampedArray<ArrayBuffer>, IMG_WIDTH, IMG_HEIGHT), 0, 0);
...
function rgbaToRgb565Bytes(rgba: Uint8ClampedArray): Uint8Array {
  const pixelCount = IMG_WIDTH * IMG_HEIGHT;
...
function rleEncode(buf: Uint8Array): Uint8Array {
  const out: number[] = [];
  const n = buf.length;
  let i = 0;
  while (i < n) {
...
export async function pngFileToRgb565Rle(file: File | Blob): Promise<Uint8Array> {
